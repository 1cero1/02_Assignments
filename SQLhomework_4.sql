...
CASE
    WHEN (d.summe10_20-c.summe5_7) < 0 THEN 'Negative'
    WHEN (d.summe10_20-c.summe5_7) > 0 THEN 'Positive'
    ELSE 'Neutral'
...
ELSE
Case 
    when ((d2-d1)/(q2-q1))>0 Then 1
    when ((d2-d1)/(q2-q1))<0 then -1
    when ((d2-d1)/(q2-q1))=0 then 0
end 
